...
function getRefreshToken (instanceId) {
  ...
  }).first('refreshToken').then((row) => { return row.refreshToken })
}
...
async function getBookings(refreshToken)
{
  try {
    const {access_token} = await getAccessToken(refreshToken);
    const options = {
    ...
    const response = (await appInstance.get('bookings?withBookingAllowedActions=true')).data;
...
app.get('/bookings', async (req, res) => {
    try{
        const refreshToken = await getRefreshToken(req.query.instanceId);
        out = await getBookings(refreshToken);
        bookings = out.response;
